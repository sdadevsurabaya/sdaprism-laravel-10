                ...
                <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
                    <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                        ...
                        <div class="d-flex align-items-center gap-2 gap-lg-3">
                            <a class="btn btn-sm btn-primary" href="{{ route('onepoint_product.create') }}">Create</a>
                        </div>
                    </div>
                </div>
                                    ...
                                    <table id="product-table"
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="min-w-125px sorting">Nama Product</th>
                                                <th class="min-w-125px sorting">Brand</th>
                                                <th class="min-w-125px sorting">SKU </th>
                                                <th class="min-w-125px sorting">Harga</th>
        ...
        <script>
            $(document).ready(function() {
                $('#product-table').DataTable({
                    ...
                    ajax: {
                        url: '{{ route('product.table') }}', // Update the URL according to your route
                        type: 'GET',
                    },
                    columns: [{
                            data: null,
...

                        {
                            data: null,
                            render: function(data, type, row, meta) {
                                if (row.namaproduk !== undefined && row.kemasan !== undefined) {
                                    return row.namaproduk + ' | ' + row.kemasan;
                                } else {
                                    return '';
                                }
                            }
                        },
                        {
                            data: null,
                            render: function(data, type, row, meta) {
                                if (row.brand !== null && row.brand !== undefined && row.brand.brand !==
                                    undefined) {
            ...
            function getMenuItems(row) {
                ...
                var formEdit = "{{ route('onepoint_product.edit', ['onepoint_product' => 'id_product']) }}";
                formEdit = formEdit.replace('id_product', row.id);
                menuHtml += `
                    ...
                    if (e.value === true) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                window.location.href = "{{ url('onepoint_product') }}";
                            }
                        });
                    } else {
                        e.dismiss;
