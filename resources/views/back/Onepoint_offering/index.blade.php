                ...
                <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
                    <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                        ...
                        <div class="d-flex align-items-center gap-2 gap-lg-3">
                            <a class="btn btn-sm btn-primary" href="{{ route('onepoint_offering.create')}}">Create</a>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            $('#offering-table').DataTable({
                ajax: {
                    url: '{{ route('offering.table') }}', // Update the URL according to your route
                    type: 'GET',
                },
                columns: [
                    {
        ...
        function getMenuItems(row) {
            ...
            var formEdit = "{{ route('onepoint_offering.edit', ['onepoint_offering' => 'id_offering']) }}";
            formEdit = formEdit.replace('id_offering', row.id);
            menuHtml += `
        ...
        function actions(id) {
            ...
            Swal.fire({
                ...
                if (e.value === true) {
                    $.ajax({
                        url: "{{ url('') }}/offering-delete/" + id, // Update the URL according to your route
                        method: 'POST',
                        headers: {
                        ...
                        success: function(data) {
                            ...
                            window.location.href = "{{url('onepoint_offering')}}";
                        }
                    });
                } else {
                    e.dismiss;
