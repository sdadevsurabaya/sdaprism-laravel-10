...
@section('content')
    ...
    <div id="camera-scanner-view" class="d-block">
        <div class="qris-fullscreen-scanner" id="qrisFullscreenScanner" style="display: block;">
            ...
            <div class="qris-fullscreen-bottom-bar">
                <a href="{{ route('scan.qr') }}" class="btn btn-dark bg-opacity-75 text-white border border-secondary rounded-pill px-3 py-2 d-flex align-items-center gap-2 shadow-lg text-decoration-none">
                ...
                <div class="card shadow-sm border-0 rounded-3 overflow-hidden" id="qris-fullpage-result">
                    ...
                    <div class="card-footer bg-white border-top p-3 p-sm-4">
                        <div class="d-flex flex-column flex-sm-row gap-2">
                            <button type="button" class="btn btn-primary btn-lg flex-grow-1 py-2 fs-14px fw-semibold d-flex align-items-center justify-content-center gap-2" onclick="restartCameraScan()">
    ...
    <div id="qris-loading-overlay" class="d-none position-fixed top-0 start-0 w-100 h-100 flex-column align-items-center justify-content-center bg-dark bg-opacity-75 text-white" style="z-index: 1099; backdrop-filter: blur(4px);">
                ...
                if (cameras && cameras.length > 0) {
                    ...
                    cameras.forEach((cam, idx) => {
                        ...
                        if (dropdownList) {
                            ...
                            a.className = `dropdown-item d-flex align-items-center justify-content-between py-2 ${idx === currentCamIndex ? 'active fw-bold' : ''}`;
                            a.href = 'javascript:void(0);';
                            a.innerHTML = `<span>${cam.label || 'Kamera ' + (idx + 1)}</span> ${idx === currentCamIndex ? '<i data-feather="check" class="icon-xs ms-2"></i>' : ''}`;
