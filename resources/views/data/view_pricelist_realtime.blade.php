...
@section('title', 'Detail Pricelist Realtime')
        ...
        @else
            ...
            <nav class="page-breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Detail Pricelist Realtime</li>
                </ol>
            </nav>
            ...
            <div class="d-block d-md-none mb-4" style="padding-bottom: 76px !important;">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        <h5 class="fw-bold text-dark mb-1 fs-16px">Detail Pricelist Realtime</h5>
                        <span class="badge bg-soft-success px-2.5 py-1 rounded-pill fw-medium fs-11px">
                            <i data-feather="wifi" class="icon-xs me-1"></i> TokoSDA Realtime
                        </span>
                    </div>
                    <a href="{{ route('pricelists.realtime', ['realtime' => 1]) }}"
                        class="btn btn-primary btn-sm rounded-pill px-3 d-flex align-items-center gap-1 shadow-sm">
                ...
                <div class="mb-3">
                    <div class="input-group shadow-sm rounded-3 overflow-hidden">
                        <span class="input-group-text bg-white border-end-0 text-muted"><i data-feather="search"
                                class="icon-sm"></i></span>
                        <input id="mobileGlobalSearch" type="text" class="form-control border-start-0 ps-0 fs-14px py-2"
                            placeholder="Cari item dalam pricelist ini...">
                    </div>
                </div>

                {{-- Standalone Mobile Item Cards --}}
                ...
                <div id="mobileSinglePaginationWrapper" class="d-flex flex-column align-items-center gap-2 mt-4">
                    ...
                    <nav>
                        <ul id="mobileSinglePaginationNav"
                            class="pagination pagination-sm mb-0 flex-wrap justify-content-center"></ul>
                    </nav>
                </div>
            </div>
            ...
            <div class="d-none d-md-block">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="d-flex align-items-center gap-3">
                        <h4 class="fw-bold mb-0 text-dark">Pricelist</h4>
                        <span class="badge bg-soft-success px-3 py-1.5 rounded-pill fs-12px fw-semibold">
                            <i data-feather="wifi" class="icon-xs me-1"></i> Realtime
                        </span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <a href="{{ route('pricelists.realtime', ['realtime' => 1]) }}"
                            class="btn btn-outline-primary d-flex align-items-center gap-1">
                            <i data-feather="refresh-cw" class="icon-xs"></i> Perbarui
                        </a>
    ...
    <script>
        $(function() {
            ...
            function renderMobileCards() {
                ...
                const filteredRows = dt ? dt.rows({
                    search: 'applied'
                }).data().toArray() : dataConverted;
                ...
                if (pageRows.length === 0) {
                    container.innerHTML =
                        `<div class="card border-0 shadow-sm rounded-3 text-center py-4 text-muted bg-white"><div class="card-body"><i data-feather="inbox" class="mb-2 text-secondary" style="width: 36px; height: 36px;"></i><p class="mb-0 fs-13px">Tidak ada item ditemukan.</p></div></div>`;
                } else {
                    pageRows.forEach((row, idx) => {
                        ...
                        headerLabels.forEach((label, i) => {
                            ...
                            if (!titleHtml && (label.toLowerCase() === 'name' || label
                                    .toLowerCase() === 'nama' || label.toLowerCase() ===
                                    'description' || label.toLowerCase() === 'title')) {
                                titleHtml =
                                    `<h6 class="fw-bold text-dark mb-2 fs-14px"><i data-feather="box" class="icon-sm me-1.5 text-primary"></i>${val}</h6>`;
                            } else {
                                detailsHtml += `
                        ...
                        if (!titleHtml) {
                            titleHtml =
                                `<h6 class="fw-bold text-dark mb-2 fs-14px"><i data-feather="box" class="icon-sm me-1.5 text-primary"></i>Item #${cardNum}</h6>`;
                        }
